namespace DoorRestartSystem
{
    ...
    public class Config
    {
        ...
        public void Validate()
        {
            ...
            {
                Logger.LogWarn(nameof(Config), $"Relational Variance Handled: DurationMin ({DurationMin}s) limits bound exceeded DurationMax ({DurationMax}s). Realigning nodes...");
            ...
            {
                Logger.LogWarn(nameof(Config), $"Relational Variance Handled: DelayMin ({DelayMin}s) limits bound exceeded DelayMax ({DelayMax}s). Realigning nodes...");
            ...
            {
                Logger.LogWarn(nameof(Config), $"Hardware Spectrum Error: FlickerFrequency ({FlickerFrequency}) must evaluate to strictly positive channels. Restoring defaults (2.5f).");
